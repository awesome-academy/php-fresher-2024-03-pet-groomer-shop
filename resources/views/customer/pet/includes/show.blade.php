<x-modal :title="__('Show Pet')" :btnText="__('Show Pet')" btnClass="btn-sm btn-success">
    <form enctype="multipart/form-data" method="POST" class="w-full flex flex-col md:grid grid-cols-12 ga-2 md:gap-4"
        ...
        <div class="col-span-12">
            ...
            @if ($pet->image->image_path ?? false)
                <img class="w-24 h-24 my-4 rounded-md shadow-sm"
                    src="{{ asset('storage/' . $pet->image->image_path) }}" alt="user_avatar">
            @else
