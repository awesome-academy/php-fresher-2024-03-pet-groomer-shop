     ...
     <form action="{{ route('employee.assign-task', ['employee' => $userID, 'order' => $order->order_id]) }}"
         ...
         <div class="flex flex-col flex-wrap md:grid grid-cols-12 gap-3 ">
             ...
             <div class="col-span-6">
                 ...
                 <x-input id="from_time" class="block mt-1 w-full" type="datetime-local" name="from_time" :value="old('from_time')"
                     required autofocus />
             </div>
             <div class="col-span-6">
                 <x-label required for="to_time" :value="__('To Time')" />
                 <x-input required id="to_time"  class="block mt-1 w-full" type="datetime-local" required name="to_time" />
